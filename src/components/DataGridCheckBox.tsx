...
interface DataGridProps<T> {
  ...
  onDelete?: (row: T) => void;
  onRevert?: (row: T) => void;
  onSelectionChange?: (selectedRows: T[]) => void; // 🔹 New Prop
}

function DataGridCheckBox<T>({
  columns,
  data,
  pageSize = 4,
  currentPage = 1,
  ...
  const updateSelection = (newSelectedRows: Set<number>) => {
    setSelectedRows(newSelectedRows);
    const selectedData = Array.from(newSelectedRows).map(i => data[i]);
    onSelectionChange?.(selectedData); // 🔹 Trigger callback
  };
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      paginatedData.forEach((_, index) => newSelectedRows.add(startIndex + index));
    }
    updateSelection(newSelectedRows);
  };
  ...
  const handlePageChange = (page: number) => {
    ...
    onPageChange?.(page);
    updateSelection(new Set()); // Clear selection on page change
  };
  ...
  const showActions =
    controls.view || controls.edit || controls.delete || controls.revert;
      ...
      <ScrollArea className="h-full">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                <th className="w-[40px] p-2 text-center bg-blue-600 text-white sticky top-0 z-10">
                  <input
                    type="checkbox"
                ...
                {controls.view && <th className="w-[40px] p-2 sticky top-0 z-10 bg-blue-600 text-white text-center text-sm"></th>}
                {controls.edit && <th className="w-[40px] p-2 sticky top-0 z-10 bg-blue-600 text-white text-center text-sm"></th>}
                {controls.delete && <th className="w-[40px] p-2 sticky top-0 z-10 bg-blue-600 text-white text-center text-sm"></th>}
                {controls.revert && <th className="w-[40px] p-2 sticky top-0 z-10 bg-blue-600 text-white text-center text-sm"></th>}
              </tr>
            </thead>
            <tbody>
                  ...
                  >
                    ...
                    {controls.edit && (
                      <td className="p-1 text-center border-b border-[#d1d5db]">
                        <button
                          onClick={() => onEdit?.(row)}
                          className="bg-[#2525bd] text-white p-1.5 rounded-full"
                        >
                    ...
                    {controls.view && (
                      <td className="p-1 text-center border-b border-[#d1d5db]">
                        <button
                          onClick={() => onView?.(row)}
                          className="bg-green-600 text-white p-1.5 rounded-full"
                        >
                    ...
                    {controls.revert && (
                      <td className="p-1 text-center border-b border-[#d1d5db]">
                        <button
                          ...
                          className="bg-orange-600 text-white p-1.5 rounded-full"
                        >
