...
import SelectedHandOver from './SelectedHandOver';
import HandOverDone from './HandOverDone';

interface DocumentHandoverSystemProps {
...
const DocumentHandoverSystem: React.FC = () => {
  ...
  const handleDocumentSelect = (batchGenerationId: string, isSelected: boolean) => {
  ...
  const onBulkHandOverClick = () => {
    ...
    selectedDocumentIds.forEach(batchGenerationId => {
      const doc = mockDocuments.find(d => d.BatchGenerationId === batchGenerationId);
      if (doc) {       
        const batchList = [
          {
        ...
        axios.post(`${API_CONFIG.BaseUrl}${API_CONFIG.BaseUrlWrapper}`, curdInput)
          .then(response => {
                ...
                {getCurrentPageItems().length > 0 ? (
                  getCurrentPageItems().map((doc) => (
                    <DocumentCard
                      ...
                      isSelected={selectedDocumentIds.includes(doc.BatchGenerationId)}
                    />
                  ))
                ) : (
